...
print(ggplot(df_prop, aes(x = cluster, y = prop, fill = color)) + geom_bar(stat='identity') + scale_y_continuous(expand = c(0,0), name = "") + xlab("") + theme_classic() + theme(axis.text = element_text(size = 10)) + geom_hline(xintercept = 50, linetype = "dashed", color = "gray40") + geom_hline(xintercept = 25, linetype = "dashed", color = "gray60") + geom_hline(xintercept = 75, linetype = "dashed", color = "gray60") + scale_fill_identity())
ggsave(paste0(samc_folder, mz.dataset, "_", mm.dataset, "_prop.pdf"), width = length(relative_prop)*0.15, height = 2.5, limitsize = F)
